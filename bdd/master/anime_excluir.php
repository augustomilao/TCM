		...
		<?php 
				...
				if(isset($_POST["cancelar"])){	
				 ...
				 if(isset($_POST["excluir"])){
					$nome = $_POST["nome"];
					$genero = $_POST["genero"];
					 $query = mysqli_query($cnn,"DELETE FROM animes WHERE nome = '$nome'") or die(mysql_error());
					 echo "<script> alert('Anime excluido!'); </script>";
				...
				if(isset($_POST["acao"])){
					switch ($_POST["acao"]){
						...
						$query = mysqli_query($cnn,"SELECT * FROM animes WHERE nome = '$nome' and genero = '$genero'") or die(mysql_error());            
							...
							if($result == 0){
								echo "<script> alert('Sem animes para excluir!!'); </script>";
								echo "<script> window.location='master.php';</script>"; 					
							}
							else{
						  ...
						  <div id="tudo" style="border:2px black solid; margin: 80px 20%; padding-bottom: 20px ">
								...
								<h2 class="form-signin-heading">Dados do Anime - Consultar</h2>
								...
								<label for="inputepisodio"><strong>Episodio:</strong></label>
								<input type="text" name="episodio" id="episodio" class="form-control" style ='width:auto;text-align:center' value="<?php echo $episodio;?>" readonly /></br><br>
								<label for="inputdatalanc"><strong>Data de Lançamento:</strong></label>
								<input type="text" name="datalanc" id="datalanc" class="form-control" style ='width:auto;text-align:center' value="<?php echo $datalanc;?>" readonly /></br><br>	
