      ...
      <div class="jumbotron" style ='font-size:16px'>
		<?php 
				...
				if(isset($_POST["alterar"])){	
					...
					$query = mysqli_query($cnn,"UPDATE animes SET nome='$nome', genero='$genero', episodio='$episodio', datalanc ='$datalanc' WHERE nome = '$nome'") or die(mysql_error());
					echo "<script> alert('Anime Alterado!!'); </script>";
				...
				if(isset($_POST["acao"])){
					switch ($_POST["acao"]){
						...
						$query = mysqli_query($cnn,"SELECT * FROM animes WHERE nome = '$nome' and genero = '$genero'") or die(mysql_error());       
							...
							if($result == 0){
								echo "<script> alert('Sem animes com esse nome e gênero!!'); </script>";
								echo "<script> window.location='master.php';</script>"; 	
							}
							else{
								...
								<div id="tudo" style="border:2px black solid; margin: 80px 20%; padding-bottom: 20px ">
									...
									<h2 class="form-signin-heading">Dados do Anime - Alterar</h2>
									...
									<input type="text" name="genero" id="genero" class="form-control" style ='width:auto;text-align:center' value="<?php echo $genero;?>"  /></br><br>
									<label for="inputepisodio"><strong>Episódios:</strong></label>
									<input type="text" name="episodio" id="episodio" class="form-control" style ='width:auto;text-align:center' value="<?php echo $episodio;?>" /></br><br>
									<label for="inputdatalanc"><strong>Data de Lançamento:</strong></label>
									<input type="text" name="datalanc" id="datalanc" class="form-control" style ='width:auto;text-align:center' value="<?php echo $datalanc;?>" /></br><br>	
										<br>
										<br>
