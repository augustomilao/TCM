		...
		<?php 
				...
				if(isset($_POST["cancelar"])){	
				 ...
				 if(isset($_POST["sub1"])){
					...
					$query = mysqli_query($cnn,"INSERT INTO animes VALUES ('$id','$nome','$genero','$episodio','$datalanc')");
					echo "<script>alert('Anime Cadastrado!!');</script>";
					}
				...
				if(isset($_POST["acao"])){
					switch ($_POST["acao"]){
						case "consultar":
							...
							$query = mysqli_query($cnn,"SELECT * FROM animes WHERE nome = '$nome'") or die(mysql_error());       
							$result = mysqli_num_rows($query);
							if($result == 0){?>
								<form class="form-signin" name="form1" id="form1" method="POST" action="#">
								<h2 class="form-signin-heading">Novos Animes!</h2>
								...
								<input type="text" name="episodio" id="episodio" class="form-control" style ='width:150px;text-align:center'    /></br><br>		
                                <label for="inputDatalanc"><strong>Data de Lançamento:</strong></label>
								<input type="text" name="datalanc" id="datalanc" class="form-control" style ='width:150px;text-align:center'    /></br><br>
						  ...
						  <div id="tudo" style="border:2px black solid; margin: 80px 20%; padding-bottom: 20px ">
								<form class="form-signin" name="form1" id="form1" method="POST" action="#">
								<h2 class="form-signin-heading">Anime já cadastrado!</h2>
